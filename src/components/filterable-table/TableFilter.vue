...
export default {
  ...
  setup() {
    ...
    console.log(tableFilterOptions.value)
    return { children: tableFilterOptions }
  },
  mounted() {
    console.log(this.children)
    // console.log(this.$refs.tableFilterOptions);
    // this.children = this.$refs.tableFilterOptions
  },
  computed: {
      ...
      if (this.config.optionToPinAtTheTop) {
        this.config.optionToPinAtTheTop.forEach((filter) => {
          if (filter.filterName === this.title.trim()) {
            ...
            const optionToPin = this.options.find((option) => option === filter.option)
            optionsListToDisplay = optionsWithoutPinnedOption
            optionsListToDisplay.unshift(optionToPin)
          }
        })
      }
      optionsList = optionsListToDisplay.length == 0 ? this.options : optionsListToDisplay
      return optionsList
    }
  },
  ...
  methods: {
    openSelect() {
      ...
      if (this.isOpen) {
        ...
        this.isOpen = true
        this.$emit('clickDropdown', this.name, this.tableId)
