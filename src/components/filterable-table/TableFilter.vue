<template>
  <div v-if="hasOptions" class="filter" :style="cssVariables">
    ...
    <div class="options" :class="{ active: isOpen }">
      ...
      <div class="buttons">
        <button @click="clear" class="button--clear" v-text="'Clear'" />

        <button @click="cancel" class="button--cancel" v-text="'Cancel'" />

        <button @click="apply" class="button--apply" v-text="'Apply'" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    config() {
      const config = this.$store.getters["filterableTable/options"](
        this.tableId
      );
    ...
    filterClass() {
      return (
        "filter__options--" + this.name.replace("_| |(|)", "-").toLowerCase()
      );
    },
      ...
      if (this.config.optionToPinAtTheTop) {
        this.config.optionToPinAtTheTop.forEach((filter) => {
          if (filter.filterName === this.title) {
            ...
            const optionToPin = this.options.find(
              (option) => option === filter.option
            );
            optionsListToDisplay = optionsWithoutPinnedOption;
            optionsListToDisplay.unshift(optionToPin);
          }
        });
      }
      optionsList =
        optionsListToDisplay.length == 0 ? this.options : optionsListToDisplay;
      return optionsList;
    },
  },
  ...
  methods: {
    ...
    cancel() {
      ...
      this.children.forEach((child) => {
        child.isSelected = this.activeOptions.includes(child.option)
          ? true
          : false;
      });
